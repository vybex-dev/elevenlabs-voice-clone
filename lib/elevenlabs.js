...
// ---------------------------------------------------------------------------
// ElevenLabs' PVC captcha error bodies look like:
//   { "detail": { "type": "invalid_request", "code": "bad_request",
//                 "message": "...", "status": "max_verification_attempts_reached" } }
// `detail.status` (not `detail.code`) is the stable, machine-readable value —
// pull it out when present and attach it to the thrown Error as `.code` so
// callers (the API routes) can branch on specific conditions, like the
// daily/attempt-count cap, without parsing error message text themselves.
// ---------------------------------------------------------------------------
function elevenlabsErrorCode(errorText) {
  try {
    const parsed = JSON.parse(errorText);
    return parsed?.detail?.status || parsed?.detail?.code || null;
  } catch {
    return null;
  }
}
...
export async function getPvcCaptcha({ voiceId }) {
  ...
  if (!response.ok) {
    const errorText = await response.text().catch(() => "");
    const error = new Error(`ElevenLabs API error (${response.status}) fetching captcha: ${errorText}`);
    error.code = elevenlabsErrorCode(errorText);
...
export async function verifyPvcCaptcha({ voiceId, recordingBlob, filename = "captcha-recording.webm" }) {
  ...
  if (!response.ok) {
    const errorText = await response.text().catch(() => "");
    const error = new Error(`ElevenLabs API error (${response.status}) verifying captcha: ${errorText}`);
    error.code = elevenlabsErrorCode(errorText);
    error.status = response.status;
    throw error;
  }
